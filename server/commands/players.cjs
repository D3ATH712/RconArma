...
module.exports = async ({ message }) => {
  ...
  try {
    ...
    if (playerCount === 0) {
      return message.channel.send(`📋 **Players on ${guildConfig.displayName || serverId}: 0 online**`);
    }
    ...
    const generateEmbed = (pageIndex) => {
      ...
      return new EmbedBuilder()
        .setTitle(`Players on ${guildConfig.displayName || serverId}: ${playerCount} online`)
    ...
    collector.on('end', () => {
      ...
      messageReply.edit({ components: [actionRow] }).catch(() => {
        // Ignore errors when message/channel is no longer available
      });
    });
