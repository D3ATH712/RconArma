...
// Import the new player tracker functionality
const { savePlayer } = require('../player-tracker.js');
...
module.exports = async ({ message }) => {
  ...
  try {
    ...
    // Save player data to database (for both manual and autolist commands)
    console.log(`🔍 Player command executed by: ${message.author ? message.author.username : 'unknown'}`);
    console.log(`🔍 Player count found: ${entries.length}`);
    
    if (entries.length > 0) {
      try {
        let savedCount = 0;
        for (const [id, uid, name] of entries) {
          const success = await savePlayer(uid, name, id);
          if (success) savedCount++;
        }
        console.log(`💾 Successfully saved ${savedCount}/${entries.length} players to database`);
      } catch (error) {
        console.warn('Failed to save players to database:', error.message);
      }
    } else {
      console.log('ℹ️ No players to save to database');
    }
    if (playerCount === 0) {
