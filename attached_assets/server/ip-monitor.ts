  ...
  async checkForIPChange(): Promise<void> {
    try {
      ...
      if (previousIP && currentIP !== previousIP) {
        ...
        // Log IP change to system logs
        const { storage } = await import('./storage');
        await storage.createSystemLog({
          level: 'WARN',
          component: 'IP-MONITOR',
          message: `IP change detected: ${previousIP} → ${currentIP}`,
          metadata: null
        });
        ...
        await storage.createSystemLog({
          level: 'INFO',
          component: 'IP-MONITOR',
          message: `Initial IP recorded: ${currentIP}`,
          metadata: null
        });
      } else {
        ...
        await storage.createSystemLog({
          level: 'INFO',
          component: 'IP-MONITOR',
          message: `IP check completed: ${currentIP} (unchanged)`,
          metadata: null
        });
      }
  ...
  private async sendIPChangeAlert(oldIP: string, newIP: string): Promise<void> {
    ...
    for (const channelId of this.config.alertChannels) {
      try {
        ...
        console.warn(`⚠️ Removing invalid alert channel ${channelId}: ${error.message}`);
        // Don't add invalid channels to validChannels array
      }
    }
    
    // Update config with only valid channels
    this.config.alertChannels = validChannels;
    await this.saveConfig();
  }
